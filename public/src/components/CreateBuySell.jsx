...
export default function CreateBuy() {
  ...
  const [currentCoin, setCurrentCoin] = useState(
    getLocalStorage(localStorageVariable.createAds) || coinString.USDT
  );
  ...
  const renderMarketSellPrice = function () {
    const result = calcSellPrice();
    // set html
    getElementById("marketSellPrice").innerHTML =
  ...
  const calcSellPrice = function () {
    ...
    const rateDisparityFraction = math.fraction(exchangeRateDisparity);
    const buyPrice = math.add(
      ccCoinFraction,
      math
        .chain(ccCoinFraction)
        .multiply(rateDisparityFraction)
        .divide(100)
        .done()
    );
    return math.number(buyPrice);
  };
  const renderModalReview = function () {
    getElementById("modalPreviewPrice").innerHTML =
      formatStringNumberCultureUS(calcSellPrice()) + " " + currentCurrency;
